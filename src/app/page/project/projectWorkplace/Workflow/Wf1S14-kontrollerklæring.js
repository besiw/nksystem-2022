...
const Email = props => {
	...
	const getEmailTemplate = () => {
		...
		return requestNBK({
			requestConfig: allRequests.projectWorkflow.w1s14getEmail,
			body,
			dispatch
		})
			.then(res => {
	...
	const submitWf1S14 = email => {
		...
		return axios
			.post(allRequests.projectWorkflow.w1s14sendEmail.url, formData)
			.then(response => {
				backToSteps();
